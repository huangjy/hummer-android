...
import com.hummer.core.utility.HMColorUtil;
import com.hummer.core.utility.HMFloatUtil;
 ...
 */
public class HMViewBackgroundDrawable extends Drawable {
    ...
    /* Value at HMSpacing.ALL index used for rounded borders, whole array used by rectangular borders */
    private @Nullable
    HMSpacing mBorderWidth;
    private @Nullable
    HMSpacing mBorderRGB;
    private @Nullable
    HMSpacing mBorderAlpha;
    ...
    public HMViewBackgroundDrawable(Context context) {
        mContext = context;
    }
    ...
    public boolean hasRoundedBorders() {
        if (!isUndefined(mBorderRadius) && mBorderRadius > 0) {
            return true;
        }

        if (mBorderCornerRadii != null) {
            for (final float borderRadii : mBorderCornerRadii) {
                if (!isUndefined(borderRadii) && borderRadii > 0) {
                    return true;
                }
            }
        }
    ...
    public int getOpacity() {
        return HMColorUtil.getOpacityFromColor(HMColorUtil.multiplyColorAlpha(mColor, mAlpha));
    }
    ...
    public void getOutline(Outline outline) {
        ...
        if ((!isUndefined(mBorderRadius) && mBorderRadius > 0) || mBorderCornerRadii != null) {
            updatePath();
    ...
    public void setBorderWidth(int position, float width) {
        if (mBorderWidth == null) {
            mBorderWidth = new HMSpacing();
        }
        if (!HMFloatUtil.floatsEqual(mBorderWidth.getRaw(position), width)) {
            mBorderWidth.set(position, width);
            switch (position) {
                case HMSpacing.ALL:
                case HMSpacing.LEFT:
                case HMSpacing.BOTTOM:
                case HMSpacing.RIGHT:
                case HMSpacing.TOP:
                case HMSpacing.START:
                case HMSpacing.END:
                    mNeedUpdatePathForBorderRadius = true;
            }
            invalidateSelf();
    ...
    private void setBorderRGB(int position, float rgb) {
        // set RGB component
        if (mBorderRGB == null) {
            mBorderRGB = new HMSpacing(DEFAULT_BORDER_RGB);
        }
        if (!HMFloatUtil.floatsEqual(mBorderRGB.getRaw(position), rgb)) {
    ...
    private void setBorderAlpha(int position, float alpha) {
        ...
        if (mBorderAlpha == null) {
            mBorderAlpha = new HMSpacing(DEFAULT_BORDER_ALPHA);
        }
        if (!HMFloatUtil.floatsEqual(mBorderAlpha.getRaw(position), alpha)) {
    ...
    public void setRadius(float radius) {
        if (!HMFloatUtil.floatsEqual(mBorderRadius,radius)) {
    ...
    public void setRadius(float radius, int position) {
        if (mBorderCornerRadii == null) {
            ...
            Arrays.fill(mBorderCornerRadii, UNDEFINED);
        }

        if (!HMFloatUtil.floatsEqual(mBorderCornerRadii[position], radius)) {
    ...
    public float getFullBorderRadius() {
        return isUndefined(mBorderRadius) ? 0 : mBorderRadius;
    }

    public float getBorderRadius(final BorderRadiusLocation location) {
        return getBorderRadiusOrDefaultTo(UNDEFINED, location);
    }
    ...
    private void drawRoundedBackgroundWithBorders(Canvas canvas) {
        ...
        int useColor = HMColorUtil.multiplyColorAlpha(mColor, mAlpha);
        ...
        if (borderWidth.top > 0
            ...
            if (borderWidth.top == fullBorderWidth &&
                ...
                if (fullBorderWidth > 0) {
                    int borderColor = getBorderColor(HMSpacing.ALL);
                    mPaint.setColor(HMColorUtil.multiplyColorAlpha(borderColor, mAlpha));
            ...
            //In the case of uneven border widths/colors draw quadrilateral in each direction
                ...
                int colorLeft = getBorderColor(HMSpacing.LEFT);
                int colorTop = getBorderColor(HMSpacing.TOP);
                int colorRight = getBorderColor(HMSpacing.RIGHT);
                int colorBottom = getBorderColor(HMSpacing.BOTTOM);

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
                    final boolean isRTL = getResolvedLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
                    int colorStart = getBorderColor(HMSpacing.START);
                    int colorEnd = getBorderColor(HMSpacing.END);

                    final int directionAwareColorLeft = isRTL ? colorEnd : colorStart;
                    final int directionAwareColorRight = isRTL ? colorStart : colorEnd;

                    final boolean isColorStartDefined = isBorderColorDefined(HMSpacing.START);
                    final boolean isColorEndDefined = isBorderColorDefined(HMSpacing.END);
    ...
    private void updatePath() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            ...
            if (!isUndefined(directionAwareTopLeftRadius)) {
                topLeftRadius = directionAwareTopLeftRadius;
            }

            if (!isUndefined(directionAwareTopRightRadius)) {
                topRightRadius = directionAwareTopRightRadius;
            }

            if (!isUndefined(directionAwareBottomLeftRadius)) {
                bottomLeftRadius = directionAwareBottomLeftRadius;
            }

            if (!isUndefined(directionAwareBottomRightRadius)) {
        ...
        if (mBorderWidth != null) {
            extraRadiusForOutline = mBorderWidth.get(HMSpacing.ALL) / 2f;
        }
    ...
    public float getFullBorderWidth() {
        return (mBorderWidth != null && !isUndefined(mBorderWidth.getRaw(HMSpacing.ALL))) ?
                mBorderWidth.getRaw(HMSpacing.ALL) : 0f;
    }
    ...
    private int getBorderWidth(int position) {
        ...
        return isUndefined(width) ? -1 : Math.round(width);
    }
    ...
    private boolean isBorderColorDefined(int position) {
        final float rgb = mBorderRGB != null ? mBorderRGB.get(position) : UNDEFINED;
        final float alpha = mBorderAlpha != null ? mBorderAlpha.get(position) : UNDEFINED;
        return !isUndefined(rgb) && !isUndefined(alpha);
    }
    ...
    private int getBorderColor(int position) {
        ...
        return HMViewBackgroundDrawable.colorFromAlphaAndRGBComponents(alpha, rgb);
    }

    public RectF getDirectionAwareBorderInsets() {
        final float borderWidth = getBorderWidthOrDefaultTo(0, HMSpacing.ALL);
        final float borderTopWidth = getBorderWidthOrDefaultTo(borderWidth, HMSpacing.TOP);
        final float borderBottomWidth = getBorderWidthOrDefaultTo(borderWidth, HMSpacing.BOTTOM);
        float borderLeftWidth = getBorderWidthOrDefaultTo(borderWidth, HMSpacing.LEFT);
        float borderRightWidth = getBorderWidthOrDefaultTo(borderWidth, HMSpacing.RIGHT);

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && mBorderWidth != null) {
            final boolean isRTL = getResolvedLayoutDirection() == View.LAYOUT_DIRECTION_RTL;
            float borderStartWidth = mBorderWidth.getRaw(HMSpacing.START);
            float borderEndWidth = mBorderWidth.getRaw(HMSpacing.END);
            ...
            if (!isUndefined(directionAwareBorderLeftWidth)) {
                borderLeftWidth = directionAwareBorderLeftWidth;
            }

            if (!isUndefined(directionAwareBorderRightWidth)) {
