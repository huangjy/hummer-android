...
public class RoundImageView extends ImageView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mRoundRadius <= 0) {
            super.onDraw(canvas);
        } else if (mRoundRadius >= Math.min(mDrawableRect.height() / 2, mDrawableRect.width() / 2)) {
            canvas.drawCircle(getWidth() / 2, getHeight() / 2, mCircleRadius,
                    mBitmapPaint);
        } else {
            Path path = new Path();
            path.addRoundRect(mDrawableRect, mRoundRadius, mRoundRadius, Path.Direction.CW);
        ...
        if (mBorderWidth > 0) {
            if (mRoundRadius <= 0) {
                ...
                float radius = mCircleRadius - mBorderWidth/2;
                canvas.drawCircle(getWidth() / 2, getHeight() / 2, radius,
                        mBorderPaint);
            } else {
                Path path = new Path();
                path.addRoundRect(mBorderRect, mRoundRadius, mRoundRadius, Path.Direction.CW);
    ...
    private void setup() {
        ...
        mCircleRadius = Math.min(mDrawableRect.height() / 2,
